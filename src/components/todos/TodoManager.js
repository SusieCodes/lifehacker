...
export const getTodosByUserId = (userId) => {
  return fetch(
    `${remoteUrl}/todos/?userId=${userId}&_sort=byWhen&_order=asc`
  ).then((res) => res.json());
};

export const getTodoById = (TodoId) => {
  return fetch(`${remoteUrl}/todos/${TodoId}`).then((res) => res.json());
};

export const getAllTodos = () => {
  return fetch(`${remoteUrl}/todos?_sort=byWhen&_order=asc`).then((res) =>
    res.json()
  );
};
